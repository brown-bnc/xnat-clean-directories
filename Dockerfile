...
CMD  /usr/bin/find $PARENT_PATH/home/logs -type f -name '*.log.*' -mtime +$DAYS_TO_CLEAN \ 
     && /usr/bin/find $PARENT_PATH/cache -type f -mtime +$DAYS_TO_CLEAN  \
     && /usr/bin/find $PARENT_PATH/build -type f -mtime +$DAYS_TO_CLEAN 